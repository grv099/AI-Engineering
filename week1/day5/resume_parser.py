...
#part 1: Define a Pydantic model to represent the structured information we want to extract from the job description.
...
class Resume(BaseModel):
    ...
    total_experience_years: float | None=None #optional field to store total experience in years
    skills: list[str] = []
    experiences: list[Experience] = []
    education: list[str] = []
    projects: list[str] = []
    certifications: list[str] = [] #removed the optional field for certifications as it is not needed in this context
...
def  final_score(job,resume):
    ...
    response = client.chat.completions.create(model=my_model, messages=messages, response_format=response_format)
    ...
    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
                if cell.text.strip():  # Check if the cell has non-whitespace text
                    text += cell.text + "\n"
    return text
...
for file_path in resume_folder.iterdir():
    ...
    parsed_resume = parse_resume(resume_text) #llm call
    time.sleep(5)
    result = final_score(job, parsed_resume) #llm call
    time.sleep(5)
